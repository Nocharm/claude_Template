...
이 가이드는 본 폴더의 `CLAUDE.md` + `rules/*.md` 허브 구조 템플릿을 새 프로젝트에 적용하는 방법을 설명한다. 받는 쪽 Claude Code 가 그대로 따라할 수 있도록 작성되었다.
...
- 일반 규칙은 `rules/` 직속, 언어 규칙은 `rules/languages/` 에 있다.
...
- Python 만 사용: `@rules/languages/typescript.md` 줄 삭제
- TypeScript 만 사용: `@rules/languages/python.md` 줄 삭제
- 둘 다 사용: 두 줄 유지
...
- [ ] 2. 사용하지 않는 `@rules/languages/*.md` 줄 삭제
